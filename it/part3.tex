...
Modello: \bftt{recap-exercise-solution.pdf}}}
\end{center}
\end{enumerate}
...
\item Per mostrare un \keystrokebftt{\%}, fai \emph{escape} con un backslash (\cmdbs{\%}).
\item Ricordati di inserire gli accenti con \cmd{\bs`} e \cmd{\bs'}
\item O carica il pacchetto \cmd{inputenc} con opzione \cmd{utf8}
...
\item Per decidere in che diapositive mostrare un contenuto, si impostano le \structure{overlay specification} con \keystrokebftt{<}\texttt{\ldots}\keystrokebftt{>}.
\item Per esempio \mintinline{latex}|\only<2->{...}| mostra il contenuto dalla slide~2 in avanti, \mintinline{latex}|\uncover<1,3>{...}| solo nella~1 e nella~3. Provate a trovare le differenze tra i due comandi\ldots
\end{itemize}
\end{frame}
